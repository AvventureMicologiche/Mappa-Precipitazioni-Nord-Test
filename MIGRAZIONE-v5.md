...
**(b) Coprire il guasto tipico di MeteoHub, che NON è il giorno mancante.** MeteoHub non si spegne, si svuota: il 27/7 la Puglia aveva un giorno *presente* con 0 stazioni buone su 128, la Basilicata 50 su 61, il Molise 23 su 28. `check-meteohub-gaps.js` fa la cosa giusta per la mappa — integra le stazioni mancanti dentro il file marcandole `om:true` e lascia `source: meteohub…` perché il dato reale che c'è resta reale. Ma `check-fonti.js` decide "questo giorno è reale" proprio guardando quel `source`: vedrebbe un file buono e **non suonerebbe mai**, nemmeno con una stazione al giorno per un mese.

> Sul Nord la scelta di ignorare il numero di stazioni è giusta (reti stabilissime, il guasto è il giorno che sparisce). Al Sud è rovesciata, e senza la (b) la regione con la fonte meno affidabile del progetto sarebbe l'unica senza allarme.

**Come farlo senza duplicare logica:** far leggere a `check-fonti.js` il registro `data/meteohub-gaps.json`, che quei giorni parziali li ha già misurati e datati. Regola: se una rete MeteoHub ha **3 giorni consecutivi** con eventi (`mancante` o `parziale`) che finiscono a ieri, parte la mail. Nessuna soglia nuova da tarare, nessun rischio di falsi allarmi in più, e si sfrutta una misura già validata sul campo.
